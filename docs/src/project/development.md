...
The flake pins SBCL, `cl-host-kit`, `cl-prolog-kit`, and `cl-weave`, so this path
...
`cl-prolog-kit` is used by the test system to express and verify cross-boundary
...
`sbcl --script run-tests.lisp` there when `cl-host-kit`, `cl-prolog-kit`, and
