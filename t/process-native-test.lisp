...
(it
  ...
  (expect
    ...
    :to-be-truthy)
  (expect
    (string= "" (cl-boundary-kit::%slurp-stream (make-string-input-stream "")))
    :to-be-truthy))
...
(it
  ...
  (expect
    (null
      (member
        :timed-out
        (cl-boundary-kit::%make-native-process-result '("prog") "out" "" 0)))
    :to-be-truthy))
  ...
  (it "native-process-timeout-does-not-block-on-a-descendant-owned-pipe"
    (uiop:with-temporary-file (:stream stream
      ...
      (let ((pid nil))
        (unwind-protect
             (let* ((process (make-process-boundary))
                    (command (format nil "sleep 30 & echo $! > ~S; wait"
                                     (namestring pid-path)))
                    (start (get-internal-real-time))
                    (result (process-boundary-run
                             process "/bin/sh"
                    ...
                    (elapsed-seconds
                     (/ (- (get-internal-real-time) start)
                        internal-time-units-per-second)))
               (expect (getf result :timed-out) :to-be-truthy)
               (expect (< elapsed-seconds 8) :to-be-truthy))
...
(it
  ...
  (expect
    (search "[NIL]" (%run-native-reporting '("PATH") :environment nil))
    :to-be-truthy))
...
(it
  ...
  (expect
    (search
      "[hello][NIL]"
      (%run-native-reporting '("MARKER" "PATH") :environment '(("MARKER" . "hello"))))
    :to-be-truthy))
...
(it
  ...
  (let ((ok
    ...
    (expect
      (eq t (process-result-success-p (process-boundary-run ok "cmd")))
      :to-be-truthy)
    (expect
      (null (process-result-success-p (process-boundary-run bad "cmd")))
      :to-be-truthy)))
  ...
  (it
    ...
    (let ((result
          (process-boundary-run
            ...
            :timeout
            nil)))
      (expect (eql 7 (getf result :exit-code)) :to-be-truthy)
      (expect (search "failure" (getf result :stderr)) :to-be-truthy)
      (expect (null (getf result :timed-out)) :to-be-truthy)))
