...
(describe "cache"
  (before-each
    (setf *cache* (make-test-cache)))

  (it "test-cache-stores-and-reads-values-with-present-p"
    (with-soft-assertions
      (expect (cache-put *cache* "k" 1) :to-be 1)
  ...
  (it "test-cache-expires-entries-once-the-ttl-passes"
    (let* ((now 0)
      ...
      (expect (cache-get cache "k") :to-equal "v")
      (setf now 9)
      (expect (cache-get cache "k") :to-equal "v")
      (setf now 10)
      (multiple-value-bind (value present) (cache-get cache "k" :gone)
        (expect value :to-be :gone)
        (expect present :to-be-null))))
  ...
  (it "test-cache-evict-removes-an-entry-and-reports-presence"
    (cache-put *cache* "k" 1)
    (expect (cache-evict *cache* "k") :to-be t)
    (expect (cache-evict *cache* "k") :to-be-null)
    (multiple-value-bind (value present) (cache-get *cache* "k" :gone)
  ...
  (it "test-cache-accepts-non-expiring-initial-entries"
    (let ((cache (make-test-cache :initial '(("k" . 1)))))
      (expect (cache-get cache "k") :to-be 1)))
  ...
  (it "make-cache-wires-injected-collaborators"
    (let* ((events '())
      ...
      (cache-get cache "k")
      (cache-evict cache "k")
      (expect events :to-equal (list (list :evict "k") (list :get "k") (list :put "k" 7 3)))))
  ...
  (it-each ((recording-cache-calls)
      ...
      (operation)
    (expect (lambda () (funcall operation (make-test-cache))) :to-throw "Unsupported cache type"))
  ...
  (it "cache-fetch-computes-and-stores-a-value-only-on-a-miss"
    (let ((calls 0))
      (flet ((compute () (incf calls) "value"))
        (expect (cache-fetch *cache* "k" #'compute) :to-equal "value")
  ...
  (it "test-cache-clear-empties-the-cache"
    (let ((cache (make-test-cache :initial '(("a" . 1) ("b" . 2)))))
      (expect (cache-clear cache) :to-be cache)
  ...
  (it "recording-cache-records-clear"
    (let ((cache (make-recording-cache :delegate (make-test-cache :initial '(("a" . 1))))))
      ...
      (expect (recording-cache-calls cache)
              :to-have-recorded-calls
              (list (boundary-call-plist :clear '() :result t)))))
