...
;;;;
;;;; Advanced cl-prolog-kit usage: bounded parsing of untrusted policy source,
...
(it "untrusted-policy-source-parses-and-answers-queries"
  (let ((policy (cl-prolog-kit:consult-prolog *boundary-policy-source*)))
    (expect (cl-prolog-kit:prolog-succeeds-p
             policy (cl-prolog-kit:read-prolog-term "permitted(filesystem, read)"))
            :to-be-truthy)
    (expect (cl-prolog-kit:prolog-succeeds-p
             policy (cl-prolog-kit:read-prolog-term "permitted(filesystem, mutate)"))
            :to-be-null)))
...
(it "prolog-query-boundary-rejects-unbound-goals-with-a-catchable-error"
  (handler-case
      (progn
        (cl-prolog-kit:query-prolog *boundary-policy* '?goal)
...
(it "prolog-query-boundary-rejects-non-callable-goals-before-dispatch"
  (handler-case
      (progn
        (cl-prolog-kit:query-prolog *boundary-policy* 42)
...
(cl-prolog-kit/weave:deftest-queries boundary-delegation-reachability-terminates
    (*boundary-delegation*)
...
(it "tabled-reachability-over-a-cyclic-graph-terminates-and-deduplicates"
  (handler-case
      (let ((graph (cl-prolog-kit:consult-prolog
                    "
:- table(reaches/2).
delegates(a, b).
delegates(b, c).
...
reaches(X, Y) :- delegates(X, Z), reaches(Z, Y).")))
        ...
        (expect (= 3 (length (cl-prolog-kit:query-prolog
                              graph (cl-prolog-kit:read-prolog-term "reaches(a, Y)"))))
                :to-be-truthy))
    (error (condition)
...
;;; unqualified; DEF-DCG-RULE/MAKE-RULEBASE/PHRASE-ALL are cl-prolog-kit entry
;;; points.)

(defparameter *capability-grammar*
  (cl-prolog-kit:make-rulebase
   :clauses (list (cl-prolog-kit:def-dcg-rule effect (terminal :read))
                  (cl-prolog-kit:def-dcg-rule effect (terminal :write))
                  (cl-prolog-kit:def-dcg-rule effect (terminal :request))
                  (cl-prolog-kit:def-dcg-rule effect (terminal :observe))
                  (cl-prolog-kit:def-dcg-rule capability effect)
                  (cl-prolog-kit:def-dcg-rule capability effect capability)))
...
stream is a well-formed boundary capability grant."
  (and (member nil (cl-prolog-kit:phrase-all *capability-grammar* 'capability tokens)
               :test #'eq)
       t))
